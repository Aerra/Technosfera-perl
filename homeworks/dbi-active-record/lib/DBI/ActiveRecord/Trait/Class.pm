...
Данный класс отвечает за описание объектов ActiveRecord - к какой БД подключаться, 
какая таблица связана с объектом и какие в ней поля, какое поле является первичным ключом и т.п.
...
Имя поля, значение для которого определяется самой БД. 
Значение данного поля будет игнорироваться при запросах вставки. 
Может быть не задано. Если задано, то всегда совпадает с C<primary_key>. 
...
Список имен атрибутов объекта, которые являются полями в БД.
...
Выполняется после исполнения базового метода C<make_immutable> объекта.

Служит для валидации описания объекта. 
Вызывает исключения в случае, если валидация не пройдена. 
Например: не заданы БД, таблица или первичный ключ; первичный ключ определен более чем для одного поля; 
автоинкремент на поле не являющимся первичным ключом и т.п.

Также метод создает набор селекторов для выборки данных из БД. 
Для каждого поля С<$field>, которое определено как индексное (параметр C<index> атрибута) создается метод вида:

    "select_by_".$field

Данный метод просто вызывает метод C<select> объекта, передавая в него в качестве ключевого поля C<$field>.

Для неуникальных индексов также определяется лимит по умолчанию, 
который берется из параметра C<default_limit> атрибута. Для таких полей он является обязательным!
