...
Данный класс используется непосредственно для работы с БД. Его абстрактность обусловленна тем, 
что различные СУБД требуют различных подходов по работе с ними. Вся специфическая работа 
непосредственно с СУБД должна определяться в классах-наследниках от C<DBI::ActiveRecord::DB> 
(например, см. C<DBI::ActiveRecord::DB::SQLite>).

Данный класс реализует паттерн синглтон - объект создается один раз при первом обращении к классу, 
и используется при всех последующих обращениях. 
Доступ к этому объекту осуществляется через метод класса C<instance>.
...
Значение атрибута может быть автоматически сгенерировано путем определения 
в наследниках метода C<_build_connection_params>, который должен возвращать 
ссылку на массив с параметрами.
...
Метод класса для получения объекта. Следует использовать непосредственно вместо метода C<new>.
...
Метод для выборки из БД, вызывающийся из наследников класса C<DBI::ActiveRecord::Object>. 
C<$klass> - класс ActiveRecord объекта, который будет использован для конструирования.

Значения ключевого поля перед передачей в SQL запрос, должны сериализовываться фунцией C<serialize>, 
если она определена для данного атрибута.

Для ключевого поля должен быть определен атрибут C<index>, в противном случае будет вызвано исключение!

Значения полей полученных из БД должны быть десериализованны фунциями C<deserialize>, 
если таковые определены для этих атрибутов.
...
Значения полей перед передачей в SQL запрос, должны сериализовываться фунциями C<serialize>, 
если они определены для данных атрибутов.

Если первичный ключ автоинкрементальный и для него определена функция C<deserialize>, 
то его значение полученое из БД должно десериализовываться. 
...
Значения первичного ключа и остальных полей перед передачей в SQL запрос, должны 
сериализовываться фунциями C<serialize>, если они определены для данных атрибутов.
...
sub update {
    ...
    $self->_update($obj->meta->table_name, $key_field, $key_value, \@ok_fields, \@bind);
    return 1;
}
...
Значениe ключевого поля перед передачей в SQL запрос должно сериализовываться фунцией C<serialize>, 
если она определена для данного атрибута.
